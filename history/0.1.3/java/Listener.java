...
public class Listener
{
	...
	public static void stopOrphanedPorts(Connection conn, String targetConfig, String hostName) throws SQLException
	{
		try
		{
			loadProperties(targetConfig);
			...
			for (int i = 0; i < sessions.length-1; i++)
			{
				...
				{
					if (!checkPort(conn, sessionPort, hostName))
						killPid(pid);
				} 
			}
		}
	...
	public static void startMissingPorts(Connection conn, String targetConfig, String hostName) throws SQLException
	{
		try
		{
			loadProperties(targetConfig);
			...
			Integer portCheck = 0;
			String strSQL = getSQLForPorts(hostName, portCheck);
			if (GPLink.debug)
	...
	private static String getSQLForPorts(String hostName, Integer myPort) throws SQLException
	{
		try
		{
			if (GPLink.debug)
			...
			strSQL += "and split_part(split_part(e.location[1], '/', 3), ':', 1) = '" + hostName + "'\n";
	...
	private static boolean checkPort(Connection conn, Integer myPort, String hostName) throws SQLException
	{
		try
		{
			boolean result = false;

			String strSQL = getSQLForPorts(hostName, myPort);
			if (GPLink.debug)
				System.out.println("strSQL: " + strSQL);
